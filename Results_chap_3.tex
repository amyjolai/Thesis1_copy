...
External changes in pressure, humidity, stress and temperature cycling induce variation in the degradation patterns of energetic materials. %ref?
...
It was found that the two dominating decomposition reactions were homolysis of the nitrate ester \ce{O-NO2} bond (equation \ref{equ:homofiss}) and intramolecular elimination of \ce{HNO2} (equation \ref{equ:hno2_elim}).
%
The degradation of nitrate esters at temperatures over 100\degree C is primarily \textit{via} thermolytic processes, whilst under 100\degree C, decomposition is thought to largely be the result of hydrolysis \cite{Moniruzzaman2014}. 
...
Inspection of the products that are generated from initial processes, with observation of the species permeating through to later stages, will shed light on the most likely contributors to the autocatalysis. 
...
The acidic species proceed to react with other moieties in the system, such as unsubstituted alcohol side chains on the polysaccharide, or small molecules free in the bulk. 

When exploring the interaction of nitroglycol and nitroglycerin in acid solution, Camera proposed a protonation-denitration scheme whereby initial protonation at the nitrate is rapid, but the subsequent release of the nitronium ion was slow and rate determining (scheme \ref{sch:cameraschema}).
\begin{scheme}[htp!]
\centering
...
\ac{NC} in storage is kept wetted with solvents to prevent self-ignition. Material with 12.6\acs{pN} or lower must be stored in 25\% water by mass, or a controlled mixture of solvents, stabilisers and plasticisers. % Something about the surfaces exposed to water? Faster NO'2 release or something?
...
Despite using the author supplied geometries, same method and basis, it can be seen that the obtained PETN reaction energy in the case of homolytic fission varies greatly from the value found by Tsyshevsky \textit{et al.}
...
As the same structures were used to calculate values listed in table \ref{tab:intramolec}, the unconverged geometries therefore do not fully explain the large discrepancy between the literature energies and obtained values here.
