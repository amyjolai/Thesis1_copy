...
Though nitrate homolysis is an endothermic reaction, the weak O-N bond has a typical dissociation enthalpy of 42 kcal mol\textsuperscript{-1} and is easily cleaved when exposed to elevated temperatures, UV light or impact \textcolor{green}{\cite{Zaki2016}}. %REF UV 
...
It was found that the two most significant decomposition reactions were homolysis of the nitrate ester \ce{O-NO2} bond (equation \ref{equ:homofiss}) and intramolecular elimination of \ce{HNO2} (equation \ref{equ:hno2_elim}) \added{REF}. 
\added{[This was shown by XXX and XXX \textit{et al.} that whatever whatever process / spectroscopic technique / logic they used to show that this - did I also mentioned this in the intro?  reiterate here: Jacobs, Burke Elrod, ]}
...
Collated from by Matveev \textit{et al.}\cite{Matveev2003} \added{from the work of Andreev,K. K.  Belyaev,A. F. ** BA Lur'e, BS Svetlov }
}
...
The free energies of reaction ($\Delta$ G) were calculated according to equations \ref{equ:homofiss} and \ref{equ:hno2_elim} to reproduce the work of Tsyshevsky \textit{et al.}; The published geometries of PETN and its derivatives were obtained from the authors in a private communication. 

% Some comement in blue to say I added the equation:
\begin{scheme}[h]
\begin{equation}
\Delta G_{Homolysis} = (G_{\ce{RO^{.}}} + G_{\ce{^{.}NO2 }}) - G_{\ce{RONO2}}
\label{equ:homofiss_calc}
\end{equation}

\begin{equation}
\Delta G_{\ce{HNO2}  elim.} = (G_{\ce{R=O}} + G_{\ce{HNO2}}) - G_{\ce{RONO2}}
\label{equ:hno2_elim_calc}
\end{equation}
R = \ac{PETRIN}, NC monomer
...
Geometry optimisation and thermal calculations were to the level of 6-31+G(2df,p). \ac{NC} monomer structures were optimised using \acs{wb97xd},  %and \acs{B3LYP}, 
to default convergence criteria (max. force $4.5 \times 10^{-4}$ H/Bohr, RMS force $3 \times 10{-4}$ H/Bohr, max. displacement $1.8 \times 10{-4}$ H/Bohr and RMS displacement $1.2 \times 10{-4}$ H/Bohr. %and \ac{MP2}. 
...
\midrule
% Do I double these up?? %21.50946
\ce{^{.}NO2} loss	& 	21.50946		 &16.55804 		&	35.61579	& 	${21.50}^{b}$ 	& 16.55804 	 \\
...
\caption[Optimised protonated \ac{NC} monomer structures.]{Protonated \ac{NC} monomer structures after geometry optimisation to the level of \ac{wb97xd}/6-31+G(2df,p).
\textit{Left column:} In vacuum. 
\textit{Right column:} In \ac{PCM} implicit solvent.
The interacting parts of the molecule are highlighted in 3D, in the structures optimised in gas phase.} %, showing interaction between the proton on the bridging site with the capping group oxygen.}
...
Water as the protonating species was also attempted (figure \ref{fig:neutral_prot}), by optimisation of one, two and three water molecules in coordination with the nitrate site in the \ac{NC} monomer, however no stable complex could be isolated. It is anticipated that a much larger network of waters around both the regions surrounding the nitrate moiety and the wider molecule, are be required to achieve a stable water coordination structure. %ref that 5 water study  I jsut foudn 
This would be of interest for further investigation into the mechanism of neutral hydrolysis involving the autoionsation of water \cite{DaSilva2013}. %The optimisation of \ce{H3O} in coordination with the fully nitrated monomer was also tested, but was only possible up to the level HF/6-31g.  
