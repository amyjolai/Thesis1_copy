...
\subsubsection{Topology analysis}
QTAIM ref: A quantum theory of molecular structure and its applications (Bader, 1991)
(table \ref{tab:cps})
...
\acs{ESP} was mapped to the electron density, extracted from the formatted checkpoint file following optimisation. \acs{ESP} maps were visualised using \ac{G09}, using the CubeGen utility. Density and \ac{ESP} cubes were generated with 80 points per side. 
...
Dimers at different levels of nitration were capped with a methoxy group (-OCH\textsubscript{3}) or a hydroxyl group (-OH) at either C1 and C4 positions (Figure \ref{fig:capping-groups-all}). 
