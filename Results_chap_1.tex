...
\begin{figure}[htp]
\centering
\includegraphics[width=0.65s\linewidth]{HBEA-P15-0021-crop}
...
%    \caption{Trimer}
  \end{subfigure} 
    ...
    units, whereby each additional glucpyranose ring is added alternately in the 1$\rightarrow$4 position, as in the structure of cellulose.}
  \label{fig:models}
\end{figure}
...
Monomer, dimer and trimer structures were drawn and capped with either methoxy or hydroxyl groups, then underwent a \ac{QM} geometry optimisation. For dimer and trimer structures, individual glucopyranose rings were added at the 1$\rightarrow$4 position, with alternating planarity (figure \ref{fig:models}).
Trimer and select dimer molecules were pre-optimised with a \ac{MM} optimisation using either the UFF or MMFF94 forcefields. The dimer cases did not show any noteable improvement in the speed or frequency of attaining convergence during optimisation, or any significant difference in final optimised structures following \ac{MM} pre-optimisation. Further investigations did not implement \ac{MM} pre-optimisation for dimer geometries. 
Pre-optimisation on trimer structures reduced the number of steps required for \ac{QM} optimisation by improving the starting structure. Final optimised structures varied according to the input starting structure; in these cases the structure with lowest energy was selected. 
The numbering scheme for structures is detailed in figure \ref{fig:labelling}. Counting from the oxygen of the glucopyranose ring as atom 1, the numbering of carbons proceeds in a clockwise direction. For simplicity, the nitrate groups are numbered 1 - 3 moving clockwise. The oxygen linking the nitrate to the polysaccharide is labelled (Ox). Oxygen on the terminal ends of the nitrate group are labelled (Ot). Oxygen labelling is applied across all nitrate groups. For example, a \textit{terminal oxygen} on the nitrate at the C6 position of the \textit{second ring}, would be referred to as R2N3Ot.
...
Chain ends were capped with a combination of methoxy groups  (-OCH\textsubscript{3}), or a combination of a methoxy and a hydroxyl group (-OH) (figure \ref{fig:capping-groups-all}). The differences in charge distribution and the nature of intra-molecular interactions were probed using \ac{QTAIM}.
When considering the model size, the \ac{ESP} and topology analyses were compared across monomer, dimer and trimer models. Nitration was reduced to C2 sites to explore interactions at lower levels of nitrations, and to further simplifiy the optimisation of the trimer structure. The monomer was nitrated only at C2, the dimer nitrated only at R1C2 and the trimer nitrated at R1C2 and R3C2.
...
All electronic structure calculations, including geometry optimisation and thermodynamic calculations were performed in vacuum to the level of \acs{B3LYP}\/ 6-311+G(d,p) %with tight convergence criteria, 
using the \ac{G09} quantum chemistry suite. Minima were found using the Berny optimisation algorithm (REF), with tight convergence criteria (table \ref{tab:convergence}).
...
Structures were built using z-matrix notation or the \ac{GView} graphical interface. Molden 5.0.2 and \ac{GView} packages were used for visualisation. Avogadro molecular editor was used for \ac{MM} pre-optimisation. \ac{MM} optimisation forcield... etc
\acs{ESP} was mapped to the electron density, extracted from the formatted checkpoint file following optimisation. \acs{ESP} maps were visualised using \ac{G09}, using the CubeGen utility. Density and \ac{ESP} cubes were generated with 80\textsuperscript{3} point grids. 
...
Dimers with \ac{DOS}=3 were capped with methoxy or hydroxyl groups at C1 and C4 positions (figure \ref{fig:capping-groups-all}). The electronic and steric properties of both capping groups and the effect on the whole dimer molecule was compared. The methoxy group was expected to provide a more chemically accurate proxy for the extended polysaccharide, though the hydroxyl capping group has demonstrated  wider usage in literature. This may be due to its smaller radius which reduces distortion to the wider structure of the model and limits the introduction of steric effects. It remains relatively chemically benign and it can be argued that it corresponds instead to polysaccharide chain endings. % resf? and at slightly lower computational cost. [bit of a weak sentence]
...
\begin{table}[htp!]
\begin{center}
\caption{Topology analysis of dimers with different capping groups. *only present in CH3CH3, CH3OH **only present in CH3CH3, *** intramolecular steric centres located between R1 and R2, and between nitrate groups and nearest hydrogen on the glucpyranose ring.}
...
The topology analysis of all dimers reveals notable intermolecular interactions across the two rings, in particular where the C6 side chain is able to interact with the nearest nitrate group on the opposite ring (table \ref{tab:table_cp}). All three structures exhibited similar characteristics; \ac{NCP} lay at all nuclear sites and each chemical bond was described by a \ac{BCP}. \ac{RCP} lay at ring centres and in points of steric interference. A single \ac{CCP} was observed between R1C5 - R1C6 - R2N2 where the arrangement of the side chains constructed a pseudo cage-like environment. 

Hydrogen bonding was observed between within the nitrate groups directly connected to the ring and associated $\alpha$-hydrogens (figure \ref{fig:alpha}). N1 and N2 nitrates are orientated in the eclipsed position relative to the  $\alpha$-hydrogen, bringing the terminal oxygen within H-bonding distance and generating a steric critical point (table \ref{tab:Hconformation}). 
This was not the case for C6 nitrates, which are not directly connected to the ring. The flexbility of the C6 alkyl chain allows for gauche orientation of the \ce{CH2} group relative to the nitrate. The increased distance between the terminal oxygen of the nitrate and the $\alpha$-hydrogens exceeded H-bonding distance and redued steric crowding in the region.
