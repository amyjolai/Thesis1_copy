...
A wide range of reaction products are experimentally observed during the low temperature decomposition and slow ageing of \ac{NC}. Small gaseous species such as \ce{CO2}, \ce{CO}, \ce{CH2O}, \ce{N2O}, \ce{NO2} were identified using IR spectroscopy, whilst the presence of larger species such as glycolic acid and oxalate were revealed by analysis of the complex reaction mixture %alluded to ..spectra, suggested by 
...
Generation of these products is attributed to the numerous susceptible attack sites along the polysaccharide backbone and at side groups, as well as the multitude of possible secondary reactions following denitration. These include depolymerisation and ring-cleavage, and can involve contaminants or acids remaining in the \ac{NC} matrix following synthesis. These are left behind due to imperfect washing procedures, or produced as a result of low level ambient degradation \cite{Chin2007,Edge1990}. %(REF something about the deg products or many secondary reactions) \cite{gun'ko2014}
...
Understanding of the ageing reactions will shed light on the possible interactions between the \ac{EM} and the stabilising mixture, informing better industrial practices, safety standards and products with improved estimates of service life. 

%[*End*]%%%%%%%%%%%%%%%%%%%%%%%]
\begin{figure}[h]
\centering
\includegraphics[width=0.7\linewidth]{Cellulose}
\caption{\added{Bulk cellulose fibres are constructed from many linear strands of cellulose polymer, inter-linked by hydrogen bonds. The intermolecular bonding structure between multiple strands influence degradation properties by altering the crystallinity and solvent porosity of the bulk material. } \cite{onlinetextileacademy2016}.}
\label{fig:fibre}
\end{figure}
%number each level of struture

In addition to environmental factors and contaminants, the degradation properties of \ac{NC} depend on the \deleted{ primary, secondary and tertiary} polymer chain structure\replaced{,}{. These properties are} largely defined by the cellulose feedstock (figure \ref{fig:fibre}). 
\added{Cellulose is formed from the sequence of individual monomer units that construct the polysaccharide chain. The side groups of the monomeric sugar units determine the local intermolecular interactions between polymer strands, in this case hydrogen bonding, that lead to the fibre structure. } 
\acs{SEM} images of \textit{Miscanthus} cellulose following nitration show a bloating of the cellulose fibres, with almost full retention of the fibre structure integrity (figure \ref{fig:miscanthus} ).
...
Shukla \textit{et al.} truncated the polymer structure to a single-ring monomer for the purposes of investigating the alkaline hydrolysis behaviour of \ac{NC} using \ac{DFT} \cite{Shukla2012}.
The study analysed the \acs{sn2} nucleophilic attack at the nitrate-carbon site, leading to release of the nitrate ion in favour of a hydroxyl group (figure \ref{fig:C2_C3_C6}). Findings suggested that when starting from a fully nitrated monomer, denitration \textit{via} alkaline hydrolysis followed a sequence starting from carbon 3 (C3), followed by the carbon 2 (C2) and the carbon 6 (C6) site. 
...
%    \caption{Trimer}
  \end{subfigure} 
    ...
    The trimer was constructed from the optimised dimer, with duplication of the first ring.}
  \label{fig:models}
\end{figure}
 
 %Could probably use a linking line here
In this chapter, the electronic properties of the monomer, dimer and trimer truncations of the \ac{NC} polymer were compared. The most suitable model for subsequent investigations was determined with consideration for accurate representation of the chemistry of the full system, alongside conservation of computational effort% during the exploratory phase of the study
...
The monomer starting structure was constructed using the structure of  $\beta$-D-glucopyranose.

\deletec{Monomer, dimer and trimer structures were constructed with side chains as closely matching literature orientations as possible.} %ref
Units were capped with either methoxy or hydroxyl groups, followed by a quantum mechanical %\ac{QM}
geometry optimisation. %Ref intro section of relevance?
...
Selected dimer and trimer molecules were pre-optimised with a \ac{MM} geometry optimisation using UFF or MMFF94 forcefields to determine whether any improvements could be achieved with respect to identification of the global minimum structure or reduction of %\ac{QM}
the quantum mechanical optimisation time. 
...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.75\linewidth]{3_3_rot_label_caption}\quad
...
for main group elements and appropriate for the current model system size, where the largest trimer extends to 76 atoms\cite{bento2005,Tirado-Rives2008}. 
...
the \ac{GView} graphical user interface. Molden 5.0.2 and \ac{GView} packages were used for visualisation. %ref molden gview
Avogadro molecular editor was used for \ac{MM} pre-optimisation\cite{hanwell2012}. UFF or MMFF94 forcefields were applied with Steepest Descent algorithm\cite{Cauchy1847,Meza2010} and 500 steps. % and convergence of 10e\textsuperscript{-7}.
Electrostatic potential (\acs{ESP}) surfaces were mapped to the electron density, extracted from \ac{G09} formatted checkpoint files following optimisation. \acs{ESP} maps were visualised using \ac{GView}, using the CubeGen utility. Density and \ac{ESP} cubes were generated with 80\textsuperscript{3} point grids. 
...
Fully nitrated dimers (\acs{DOS}=3) were bi-capped with methoxy (\acs{CH3CH3}), methoxy-hydroxyl (\acs{CH3OH}) or hydroxy-methoxy (\acs{OHCH3}) groups at C1 and C4 positions (figure \ref{fig:capping-groups-all}).
...
The properties of all capping group combinations and the effect on the dimer molecule was compared. Though the hydroxyl capping group has demonstrated wider usage in literature. %REF and validate your next sentence. What other similar systems has it been used on?
 This may be due to its smaller atomic radius which minimises distortion to the wider structure of the model, limiting the introduction of artificial steric effects. The hydrogen of the hydroxyl remains relatively chemically benign and it can be argued that it corresponds to polysaccharide chain endings. % resf? and at slightly lower computational cost. [bit of a weak sentence] non-reducing end?
However, the methoxy group was expected to provide a more chemically and sterically accurate proxy for the extended polysaccharide. 

The topology analysis of all dimers revealed notable intermolecular interaction across the two rings, in particular where the C6 side chain was able to interact with the nearest nitrate group of the opposite ring. %(fig?)
...
\begin{table}[htp]
\begin{center}
\caption{Topology analysis of dimers with capping groups. * \acs{CH3CH3} and \acs{CH3OH} only; ** \acs{CH3CH3} only ; *** Intramolecular steric centres located between rings, and between nitrate groups and nearest $\alpha$-hydrogen on the ring.}
\begin{tabular}{ l c c c l} 
\toprule
Dimer & CP type & Number & Interaction centres & Intramolecular bonding\\
\hline
\multirow{4}{3em}{\acs{CH3CH3}} & NCP & 63 & All atomic nuclei & - \\ \cline{2-5}
...
\bottomrule
\multirow{4}{4em}{\acs{CH3OH}} & NCP & 60 & All atomic nuclei  & -  \\ \cline{2-5}
...
& CCP & 1 & R1C5 - R1C6 - R2N2  & -  \\
\bottomrule
\multirow{4}{4em}{\acs{OHCH3}} & NCP & 60 & All atomic nuclei  & -  \\ \cline{2-5}
...
\hline
 \multirow{4}{4em}{\acs{CH3CH3}} & R1N1 - R1N2 & 3.9871 \\
 & R1N1 - R2N3 & 4.8328 \\
 & R1N3 - R2N2 & 5.2424\\
 & R2N2 - R2N1 & 4.1421 \\
\hline
 \multirow{4}{4em}{\acs{CH3OH}} & R1N1 - R1N2 & 3.9887 \\
 & R1N1 - R2N3 & 4.8327 \\
 & R1N3 - R2N2 & 5.2371\\
 & R2N2 - R2N1 & 4.1325 \\
 \hline
 \multirow{4}{4em}{\acs{OHCH3}} & R1N1 - R1N2 & 3.9359 \\
 & R1N1 - R2N3 & 4.8208 \\
 & R1N3 - R2N2 & 5.2868\\
 & R2N2 - R2N1 & 4.1297 \\
\bottomrule
...
\centering
  ...
  \end{subfigure}
\caption{Critical points identified by \ac{QTAIM} topology analysis of NC dimer nitrated at C2 position for \textbf{a)} \ac{CH3CH3} dimer, \textbf{b)} \acs{CH3OH} dimer and \textbf{c)} \acs{OHCH3} dimer.}
\label{fig:2_0}
\end{figure}
%
...
\begin{figure}[htp!]
  ...
  \end{subfigure}
    \caption{Electrostatic potential maps of \textbf{a)} \ac{CH3CH3}, \textbf{b)} \acs{CH3OH}, \textbf{c)} \acs{CH3OH}.} % *UNITS?*
  \label{fig:ESP}
\end{figure}
%
...
The \ac{ESP} maps % for the bi-capped methoxy dimer (\ac{CH3CH3}), methoxy-hydroxyl capped dimer (\acs{CH3OH}) and hydroxy-methoxy capped dimer (\acs{OHCH3}) 
