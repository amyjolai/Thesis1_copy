...
\begin{figure}[htp]
	\centering
	\includegraphics[width=\linewidth]{3_3_rot_label}
	\caption{Numbering scheme for dimers used in this study. Ring 1 (R1) is the first ring whereby the oxygen of the glucopyranose ring is one bond separated from the glycosidic oxygen linking the two rings. Ring 2 (R2) is the glucopyranose oxygen two bonds separated from the linking oxygen.}
  \label{fig:labelling}
\end{figure}
...
Chain ends were capped with hydroxyl groups (-OH)  or methoxy groups  (-OCH\textsubscript{3}) (figure \ref{fig:capping-groups-all}). The differences in charge distribution and the nature of intra-molecular interactions were probed using \ac{QTAIM} 
...
All electronic structure calculations, including geometry optimisation and thermodynamic calculations were performed in vacuum to the level of \acs{B3LYP}\/ 6-311+G(d,p), using the \ac{G09} quantum chemistry suite. Minima were found using the Berny optimisation algorithm (REF), with tight convergence criteria (table \ref{tab:convergence}).
...
\acs{ESP} was mapped to the electron density, extracted from the formatted checkpoint file following optimisation. \acs{ESP} maps were visualised using \ac{G09}, using the CubeGen utility. Density and \ac{ESP} cubes were generated with 80\textsuperscript{3} poitn grids. 
...
Dimers with \ac{DOS}=3 were capped with a methoxy group or a hydroxyl group at either C1 and C4 positions (figure \ref{fig:capping-groups-all}). The electronic and steric properties of both capping groups and the effect on the whole dimer molecule was compared. The methoxy group was expcted to provide a more chemically accurate proxy for the extended polysaccharide, though the hydroxyl capping group was used more widely in literature. This may be due to its small size which reduces distortion to the wider structure of the model and does not introduce steric effects, whilst remaining relatively chemically benign. % and at slightly lower computational cost. 
Upon substitution of a hydroxyl terminal group for a bulkier methoxy, the geometry of the floppy nitrate groups adjusted to accommodate. It can be seen in the case of \ac{CH3CH3} in figure \ref{fig:CPs}, a), that there is an additional \ac{BCP} along the NX and NX intramolecular bonding path.
...
\begin{table}[htp]
\begin{center}
\caption{Changes in geometry with modification of the capping group in \ac{NC} dimers.}
\begin{tabular}{ l c c c } 
\toprule
Capping & Dihedral & Tight \\
\hline
 blobl &  0.000450 & 0.000015 \\
 narvao     Force & 0.000300 & 0.000010 \\
 Maximum Displacement & 0.001800 & 0.000060 \\
 RMS     Displacement & 0.001200 & 0.000040 \\
\bottomrule
\end{tabular}
...
The \ac{ESP} maps for the bi-capped methoxy dimer (\ac{CH3CH3}), methoxy-hydroxy capped dimer (\ac{CH3OH}) and hydroxy-methoxy capped dimer (\ac{OHCH3}) represent the charge density around the molecule. A more negative value (increasingly red) indicates an area of higher electron density, whilst a more positive value (increasingly blue) indicates an area of lower electron density. It can be seen that the areas with lowest electron density lie around the centre of the glucopyranose rings and at the C6 position of the side branches. This can be explained by the number of adjacent oxygens drawing the electron density away from the less electronegative alkyl groups. The most electron rich areas are the outermost oxygens of the nitrate groups.
The presence of the hydroxyl capping group promotes a small decrease in electron density around the adjacent ring, illustrated by the slight deepening of the blue shading around the ring bonded to the hydroxyl. The hydroxy group itself presents the lowest concentration of electron density, suggesting that the terminating hydrogen would be suceptible to nucleophilic attack. The oxygen of each capping group exhibits a lower concentration of electron density than that of the bridging oxygen between the two rings. This deficiency is particularly pronouced in the case of the hydroxyl group, suggesting that the methoxy group provides a more representative approximation for the extended polymer. 
...
\ac{NC} with nitrate substituion C2 wsa because ...........
