...
-\frac{\nabla^{2}}{2}+\nu_{eff}
\end{equation}
%Is the -ve appropriate - yes it cancels out with teh Veff I think? or just go with Emilia's one, that is not in A.u.? See page 129 in Leach , also
...
%Make sure you know how the above functionals differ from teh hybrids %-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
\end{equation}
%Diagram
In this study, Pople style basis sets will be employed. Split valence basis sets such as these, describe the core electrons using fewer basis functions than the interacting valence electrons; the valence electrons play a much more significant part in bonding and intermolecular interactions.  %REF % (Table 1. Examples of split valence basis sets.).40,41
An example is 6-31+G*, where the core electrons are described using a single contracted gaussian consisting of six primitive functions. The valence orbital is split into two contracted gaussians, where one is described by 3 primitive gaussians, and the other, one primitive gaussian. 
The + sign corresponds to diffuse functions, the extension of the basis function as it tails away from the nucleus. This particularly important when describing the behaviour of ions. The * indicates the addition of polarisation functions, in this case, d-functions to all non-H atoms. This mixing of orbitals facilitates a better description for the correct shape of the \ac{MO}. 
...
The \ac{QM} calculations employed in this study typically involved a geometry optimisation, followed by a frequency calculation conducted in \ac{G09}. 
...
A rigid scan consisted of a single point energy calculation of the structure at each of step the scan, rather than full relaxation of the wider system, as in relaxed scans. 
...
\ac{TS} searching is know to be difficult.
...
The sum of the diagonal terms returns teh laplacian of the electron density ($\nabla^{2}(\rho)$); evaluation of $\nabla^{2}(\rho)$ identifies the characteristic of the \ac{CP}. 
% the second derivative of the gradient (
In the context of electron density, the \ac{CP} can be classified into four types based on the number of negative eigenvalues on the Hessian (table \ref{tab:cps}):
...
\hline
Bonding (\acs{BCP}) & (3,-1) & 2 eigenvalues -ve & 2\textsuperscript{nd} order saddle point & Bonding site \\
\hline
...
The \ac{NCP} is so called as they are generally located at atomic nuclei positions. All three eigenvalues of the Hessian matrix of the function are negative, corresponding to a local maximum on the electron density landscape. The number of \ac{NCP} is usually equal to the number of atoms, though there are exceptions, such as in \ce{Li2} which has a greater number of \ac{NCP} than atoms, and for \ce{KrH+}, with a lower number. %Why???%, 
...
When two eigenvalues of the Hessian matrix of a function are negative, a second-order saddle point is present. These sites are usually located between attractive atom pairs, and so are referred to as \ac{BCP}. %The value of real space functions at BCP have great significance. %, for example the value of XXX and 
...
When none of the eigenvalues are negative, it corresponds to the local minimum. For electron density analysis, these points generally appear in the center of cage systems, such as in pyramidal structure. These are referred to as the \ac{CCP}. 
...
The gradient path linking \ac{BCP} or two local maxima can be referred to as a ``bond path''. Presence of a bond path shows an atomic interaction path available for a variety of bonding interactions, such as intra molecular bonding or H-bonding. A network of bond paths is known as molecular graph. Scrutiny of the graph returns a reliable image of the geometry of the molecule. % . Bond path can be straight line or curve, obviously for the latter case the length of bond path is longer than the sum of the distances between BCP and associated two (3,-3) CPs.
