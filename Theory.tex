...
Exploration of the \ac{PES} allows for discovery of global and local minimum energy structures, intermediate products on a reaction coordinate and transition states, through scrutiny of the %second derivative of the 
gradient at a particular set of nuclear coordinates. 
...
Thus, when choosing between different trial wavefunctions, the solution with the lowest energy is the one closest to the exact value. 
...
The Hartree-Fock approximation was the first practically applicable method for calculation of the ground-state energy of atoms with fixed nuclear positions.  
...
The Hartree approximation neglects electron-electron interaction, describing the energy of electrons as individual particles moving independently of any dynamics of other fermions in the system. 
The calculated Hartree wavefunction alone does not include any contribution from electronic exchange, and incorrectly implies that the electrons are distinguishable. 
%without the effect of the surrounding fermions. 
Fock and Slater developed this further by introducing electron exchange. %REF and when
%This assumption places a static spherical potential around the electron, derived from the average effect of the nucleus and other electrons in the system. 
...
Starting with an initial guess of the wave functions for occupied orbitals, solution of the one-electron Schr\"{o}dinger equation generates a new set of orbitals, minimising the energy of the wave function. %CHECK
This process iterates using the new orbitals for the next optmisation, until the difference between the final solution and its previous iteration falls within an acceptable threshold, thus ``self-consistent''. 
...
$N$ electrons are treated as independent particles occupying single-electron spin orbitals.

 trial wavefunctions ($\Psi_{T}$)
