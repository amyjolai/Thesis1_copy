...
Despite the long history of research and use of \ac{NC} in industry, experimental analysis has failed to distinguish the fine mechanistic details of \ac{NC} decomposition, in part owing to the variation arising from \ac{NC} source material, but also the complexity due to the interplay of many different and simultaneous degradation interactions. Electronic structure methods provide a means to %avenue for exploration/ opportunity 
...
The general form of the Hamiltonian is given b:y 
\begin{equation}
...
$E_{el}$ is the electronic energy of the system. The values of $\mathbf{q}_{i}$ are independent variables, whereas the values of $\mathbf{q}_{k}$ are parameters. %Set parameters
...
Equation \ref{equ:elec_schro} can be solved \textit{exactly} in only very few circumstances; no exact solutions can be found for problems involving three or more interacting particles, such as in the case of a helium atom possessing two electrons and one proton. %VIVA: Why?
...
$N$ electrons are treated as individual particles occupying single-electron spin orbitals and moving independently of the dynamics of any other fermions in the system. 
...
the calculated Hartree wavefunction alone does not include any contribution from electron correlation, and incorrectly implies that the electrons are distinguishable. 
Fock developed this further by introducing Slater determinantal wave functions \cite{Fock1930}. 
The effects of exchange on the Coulomb repulsion were incorporated, achieved by taking the trial wave function as a single Slater determinant. 
...
%Say what each contribution means, what each term means. then do the equations up until you get to the Fock operator, and then get it to the eigenvalue form
...
The coulomb operator, $\mathbf{J}_{j}$ %(1)$ 
...
written in terms of its core, coulomb and exchange contributions: %K is negative because of antisymmetry
\begin{equation}
...
Starting with an initial guess solutions for the occupied orbitals $\chi_{i}$, solution of the one-electron \ac{HF} eigenvalue equations generates a new set of orbitals. 
...
the energy. This process propagates using the newly generated orbitals for the next optimisation, until the difference between the final solution and its previous iteration falls within an acceptable threshold, thus, is ``self-consistent''. 
...
The forced pairing of electrons of opposing spin into a shared orbital is referred to as the Restricted scheme. %, (e.g. R\acs{B3LYP}).  You haven't described functionals yet
...
%DO I NEED ANY OF THE EQUATIONS HERE?
%\begin{figure}[h]
%\centering
%\begin{modiagram}
%\atom{left}{
...
%This diagram is essentially lifted from Introduction to Comp Chem by Jensen
%-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
There are two approaches for solving the Schr\"{o}dinger equation for a polyatomic system with many electrons.  \textit{Ab initio} methods generate solutions from ``first principles'', without information gained from experimental results. 
...
The exchange term is non-classical encompasses the exchange energy due to the antisymmetry of the wave function, and dynamic correlation of electron motion. It can be further divided into the exchange and correlation components:
\begin{equation}
\centering
...
A basis set is the collection of mathematical basis functions, used in linear combination, to construct the molecular orbitals. An individual molecular orbital can be defined as: %The individual function are derived from the hydrogen atomic orbitals, as it is 
\begin{equation}
