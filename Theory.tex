...
The general form of the Hamiltonian is given by \cite{Cramer2004}:
\begin{equation}
...
the value of $e$, $m_e$ and $\hbar$ are reduced to 1. %Give the simplified equation here?
...
Calculation of $E_{el}$ for all possible nuclear coordinates allows the construction of a \ac{PES}. % obtain transition structures for reaction involving a change in geometry, at the peak between two troughs.  
...
%In a system of two or more electrons, each electron is indistinguishable. 
The Pauli exclusion principle states that no two identical fermions, such as electrons, may simultaneously occupy the same quantum state within the same system. When considering an atom with two or more electrons, this means that no electrons may have an identical set of quantum numbers. %say what they are?
When considering two indistinguishable electrons, the wavefunction of the system must be antisymmetric with respect to the exchange of their coordiates. 
...
These antisymmetry requirements are fulfilled by expressing the wavefunction as a Slater determinant, which changes sign with permutation of the coordinates of two electrons. 
%What is a determinant anyway

In the case of a multi-electronic system, the generalised Slater determinant is as follows:
