...
%where $\mathbf{H}$ is the Hamiltonian operator and $E$ is the energy of the system. $\mathbf{H}$ is called the eigenvalue, $\Psi$ the eigenfunction and $E$ is scalar. 
...
where all electrons are encompassed by $i$ and $j$, and all nuclei by $k$ and $l$. 
%$i \rightarrow j$ encompasses all electrons and $k \rightarrow l$ all nuclei.
$\hbar$ %= \frac{h}{2\pi}$
is the reduced Planck's constant ($\hbar = \frac{h}{2\pi}$), $m_e$ is the mass of an electron, $m_k$ is the mass of the nucleus $k$, $e$ is the charge of an electron, $Z_k$ is the atomic number of $k$ and $r_{xy}$ is the distance between particles $x$ and $y$. $\nabla^{2}$ refers to the Laplacian operator, which describes the divergence of the gradient of a field.% What does this really mean, in a practical sense? 
...
The first and second terms of equation (\ref{equ:hamiltonian}) correspond to the kinetic energy of the electrons and the nuclei, respectively. Electron - nuclear attraction is described by the third term, followed by the interelectronic and internuclear repulsive terms. The attractive and repulsive terms are potential energy terms and can be described classically. The kinetic energy terms are expressed as the eigenvalue of the kinetic energy operator ($\mathbf{T}$):
...
When using \ac{au}, the electronic mass, charge and reduced Planck's constant are reduced to 1. 
Equation \ref{equ:hamiltonian} can therefore be written as:
...
The motion of elections and nuclei are coupled in real systems, as electron density flows dynamically in response to the change in nuclear position and repulsion from other electrons. The correlated motion of particles is defined by the pairwise attractive and repulsive terms of the Schr\''{o}dinger’s equation. However, this representation is unworkable owing to the complexity of considering the interdependent movement of all particles in the system. 

The Born-Oppenheimer approximation fixes nuclear positions, such that the motion of electrons and nuclei may be decoupled. The nuclear kinetic energy term of the Schr\''{o}dinger’s equation becomes then independent of the electronic term, the electron - nuclear interaction term disappears, and the nuclear - nuclear repulsive term becomes a constant.  
