...
proposed his equation in 1926, describing a quantum system using its wave function \cite{Schroedinger1926}. %Check this is accurate. Does the equation use the wave function to describe the (energy of the) system, or does the equation just describe the wave function itself?
...
$\hbar$ %= \frac{h}{2\pi}$
is the reduced Planck's constant ($\hbar = \frac{h}{2\pi} = 1.055\times10^{-34}$ Js), 
...
Exploration of the \ac{PES} allows for discovery of global and local minimum energy structures, intermediate products and transition states on a reaction coordinate, through scrutiny of the energy %second derivatives of the gradient 
%maxima and minima 
...
In the dis-allowed case of two equivalent electrons occupying the same spin-orbital, two columns would be identical \cite{Jensen2007}. %\cite{Dykstra1994}
The evaluation of the determinant would then be zero, indicating that the probability of two electrons with identical spin occupying the same orbital was zero. 
...
In practice, equation \ref{equ:elec_schro} can only be solved \textit{exactly} in very few circumstances; no exact solutions can be found for problems involving three or more interacting particles, such as in the case of a helium atom possessing two electrons and \added{a nucleus}. %VIVA: Why? - the correlated interaction between the partices which cannot be calculated exactly. 
...
%\deleted{Electron density has reduced dimensionality when compared with the wave function, 
...
\item LYP: a widely used \ac{GGA} correlation functional, developed by Lee, Yang and Parr, with empirical fitting to the helium atom. %, only based on \textit{ab initio} calculations. 
Combination with the B88 exchange functional gives the BLYP method \cite{Lee1988}. %REF
\item PBE: the exchange-correlation functional developed by Perdew, Burke and Ernzerhof derived purely from \textit{ab initio} calculations \cite{Perdew1996}. The PBE family of functionals have parameterisations optimised for different materials and interfaces, such as \added{small molecules, crystal structures and metal surface energies }\cite{Hammer1999,Perdew2008,Wu2006}. %ref PBE sol, RPBE , PBE WC
\end{itemize}
...
In \ac{DFT}, the influence of the exchange contribution to $E_{XC}$ is significantly larger than that of correlation, which only adds minor corrections \cite{Graefenstein2009}. 
...
The description of the exchange-correlation is imperfect in \ac{DFT}; the exchange and coulomb energy terms do not completely cancel out, and an energy contribution is experienced even in a one electron system. An observable manifestation of this in larger systems is the underestimation of reaction barriers \cite{Zhang2010}. 
...
\begin{equation}
E_{XC}^{B3LYP}=E_{X}^{LDA}+a_{0}(E_{X}^{HF}-E_{X}^{LDA})+a_{X}(E_{X}^{B88}-E_{X}^{LDA})+E_{c}^{LDA}-a_{C}(E_{C}^{LYP}-E_{C}^{LDA})
\end{equation}
...
\ac{B3LYP} is of particular interest in the study of \ac{NC} as it has been applied in comparable computational investigations on glucose and its analogues, which forms the base structure of the \ac{NC} monomer \cite{Schnupf2007,Momany2004}. 

There are documented cases where \ac{B3LYP} performs extremely poorly, such as in the description of $\pi\rightarrow\sigma$ structural transformations \cite{Pieniazek2008}, and medium-long range dispersion interactions \cite{Schreiner2007}. 
In the first case, no similar transitions are expected for the reactions studied in this work. When considering the medium-range electron correlation reactions that may occur for the investigated species, the distance at which the intermolecular interactions take place and at which \ac{B3LYP} performance suffers (1.5-3.5 $\AA$), is of relevance. 
...
Grimme proposed an empirical \ac{DFT} method with dispersion correction to include van der Waals interactions, described as \ac{DFT-D} \cite{Grimme2004,Grimme2006}. 
...
is also compared \cite{Wu2001}.  \added{Reproduced from the work of Chai \textit{et. al} \cite{Chai2008,Chai2008a} with permission from the PCCP Owner Societies.}} 
\label{fig:damp}
\end{figure}
The \ac{wb97xd} functional incorporates 100\% long-range exchange, 22\% short-range exchange, a modified B97 short-range exchange and B97 correlation, with empirical dispersion corrections. When the constraint $\omega$ is reduced to 0, \ac{wb97xd} is equivalent to the B97 functional with the addition of the dispersion correction. %\added{It has been successfully applied in calculations involving non-bonding interactions, and shown to outperform \acs{B3LYP} for systems involving hydrogen bonding networks \cite{Bachrach2013}.}
The performance of \ac{wb97xd} will be compared to that of \ac{B3LYP} \added{in this work} with particular attention paid to the reaction energies.  
...
A basis set is the collection of mathematical basis functions, used in linear combination, to construct the \acf{MO}. An individual \ac{MO} can be defined as: %The individual function are derived from the hydrogen atomic orbitals, as it is 
\begin{equation}
...
Basis set superposition error \acs{BSSE} is a false lowering of the energy that can occur when two species in a system with finite bases sets approach one another to form a complex. Particle A borrows the extra basis functions belonging to particle B and an artificial stabilisation is observed. The error arises from the inconsistency in treatment between the individual particles at large separations, and the complex at short distances. This is particularly pertinent when performing \ac{PES} scans where bond breaking or formation is expected, and in the consideration of reaction energies. 
The effect is particularly pronounced for smaller basis sets. Counterpoise correction can be used to circumvent \ac{BSSE}; \added{each calculation is performed twice, once using the mixed basis set of the complex and once with the separate bases of the two species, and this energy difference is subtracted from the uncorrected energies \cite{Duijneveldt1994,Mentel2014}.} However this comes at the expense of higher computational cost. Preliminary investigations into the species examined in this study showed that the use of counterpoise correction offered minimal improvements to the energy, and the additional CPU time was not proportionate to the marginal improvements. Calculations were performed at sufficiently high basis that the effects of \ac{BSSE} were insignificant, and counterpoise correction was not used for further investigations. 

%-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
\section{Implementation \& Analysis}
The \ac{QM} calculations employed in this study typically involved a geometry optimisation, followed by a frequency calculation conducted in \ac{G09} \cite{G09}. 
This section will provide an generic overview of the methods and techniques used to prepare input or analyse the optimised geometry output from a \ac{QM} calculation. 
...
%%-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
\subsection{Molecular mechanics geometry optimisation}
\added{
In the case of the larger models in this study (60 atoms and above), \ac{QM} geometry optimisation was a laborious and expensive process, particularly with the presence of numerous floppy \ce{O-H} bonds and small methyl group rotations. 
In order to reduce the required time and computational cost, these structures were optimised using a low-level molecular mechanics geometry calculation to minimise large forces in the system, prior to the finer refinements made during the \ac{QM} optimisation.
Molecular mechanics \acs{MM} uses classical Newtonian mechanics to model chemical structures and properties; the electrons are not explicitly handled, bypassing the Schr\"{o}dinger's equation \cite{Foresman1996,Jensen2007}. Molecules are instead treated as a "ball on a spring", %where Newton's second law applies, 
where the strength of interaction is defined by the atom types and force constants. 

Each \ac{MM} approach is defined by its force field, where the energy of the system is described by a parametric function, fitted to experimental or high level computational data. Different force fields are therefore fitted to different chemical systems. 

The general functional form of a force field is:
}
\begin{equation}
\added{
...
\end{equation}
\added{where  $E_{FF}$ is the energy of the system. The first three terms can be referred to as the 'bonding' terms: $E_{str}$ is the energy function for bond stretching between two atoms, $E_{bend}$ the energy required to bend a bond angle and $E_{tors}$ is the energy of rotation around a bond. The remainder are thus called 'non-bonding' terms: $E_{vdw}$ and $E_{el}$ refer to the energy of non-bonding van der Waals interactions and of electrostatic interactions, respectively. $E_{cross}$ defines the coupling of the first three terms. 

In this study, the \ac{UFF} \cite{Rappe1992} and \ac{MMFF94} \cite{Halgren1996I,Halgren1996II,Halgren1996III,Halgren1996IV,Halgren1996V} force fields are tested on \ac{NC} dimer and trimer structures. 
\ac{UFF} is an empirical, all-element model, in principle able to describe the whole periodic table. 
It has been parameterised on individual elements, hybridization and connectivity; all the atom types in the \ac{NC} unit are included, and \ac{UFF} has successfully been applied in the study of cellulose dissolution \cite{Bazooyar2012}. 
\ac{MMFF94} is a forcefield parameterised from high level \ac{QM} calculations for small molecules, organics and proteins. It has been used to sample different torsional conformers of cellulose \cite{Easton2018} and to study the dynamics of bio-active polysaccharides \cite{Imberty2000}. 
}
%-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

\subsection{Potential Energy Surface scans}
Geometry scans or \ac{PES} scans were used to probe the local energy landscape corresponding to a specific change in geometry. During the course of a scan, a structural property - such as a selected bond length, angle or dihedral is adjusted in incremental steps, as specified by the given scan parameters. In the case of a relaxed scan, at each step, the adjusted parameter is frozen and a geometry optimisation is performed, allowing the rest of the system to relax around the modified bond. 
Each scan yields a \acs{PES} of the explored pathway. In the case of bond breaking or formation, the presented energy profile is a reaction co-ordinate diagram. % (see Figure 17, section 2.4.1).
In these cases, an energy maximum followed by a trough indicates a transition state and intermediate reaction product, respectively. The structural co-ordinates at the peak of the curve can be extracted and used for subsequent frequency calculations or a transition state search, to validate the mechanistic pathway. % using intrinsic reaction co-ordinate methods. 
Intrinsic reaction coordinate calculations also test the reactant and product along the reaction pathway, to ensure that they lie on the same \ac{PES}. 
To explore the predicted degradation mechanisms of \ac{NC}, the scanned parameters were assigned to the bonds undergoing the most significant transformation during a particular step of the mechanism. In the case that more than one significant bond was altered, multiple scans with different bond specifications were compared. In this way, multi-dimensional scans could be performed. Two-dimensional scans were used to probe simultaneous processes in the system, however, these proved computationally intensive due to the high number of coordinate points to be calculated. 
Relaxed geometry scans were performed on the optimised reactant geometry using the Opt=ModRedundant keyword \added{to specify the scanned coordinate, the number of scan steps and the method for energy optimisation.} %, where a specific coordinate is chosen and incrementally adjusted at each scan step. The coordinate is then frozen whilst the remainder of the system undergoes an energy minimisation around the frozen coordinate. }% the chosen co-ordinate is frozen at each optimisation stage}.
...
Transition state searches are called through the Opt=TS, QST2 or QST3 keywords in \ac{G09}. The Opt=TS method attempts to optimise an input ``guess'' geometry to a maximum. The guess geometry can be drawn by hand, obtained \textit{via} a \ac{PES} scan, from coordinates in literature or generated using the QST2 function. 
...
Often, a \ac{TS} calculation alone will not be able to isolate the correct transition state and is usually used in conjunction the QST2 or QST3 methods. 
...
%-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
\subsection{Solvent model}
\added{
To account for solvent effects in the reactions of \ac{NC}, a \ac{SCRF} method was applied \cite{Tapia1975}. These methods treat the solvent as a uniform dielectric constant ($\epsilon$) in the background. 

The solvated molecule is placed in a cavity within the \ac{SCRF}. The solute dipole induces a dipole in the solvent medium surrounding the cavity, which in return leads to an electric field within the cavity called the reaction field. 
The shape of the cavity is dictated by the the specific \ac{SCRF} approach; in this study, the \ac{PCM} is used \cite{Miertus1981,Tomasi2005}. \ac{PCM} models the cavity as interlocking van der Waals radii around each atom. 
The charges at the van der Waals surface are split into a series of small rectangles %defined by $\Delta\theta$ and $\Delta\phi$, 
and the value of the point charge at that surface element is calculated by: }
\begin{equation}
\added{
q_i = - \left[\frac{\epsilon - 1}{4\pi\epsilon} \right]E_{i}\Delta S 
}
\end{equation} 
\added{
where $\epsilon$ is the dielectric, $E_{i}$ the electric field gradient and $\Delta S$ the area of the surface element. The coulombic contribution of each surface element to the other charges are calculated and iteratively refined until self-consistent \cite{Leach2001}. 
This calculated charge potential ($\phi_r$) is then added to the solute Hamiltonian: }
\begin{equation}
\added{
\mathbf{H} = \mathbf{H}_{0}+\phi_{r}
}
\end{equation} 
\added{ and the \ac{SCF} reaction continues as normal. New surface charges are calculated at each \ac{SCF} cycle until the overall surface charge is also self-consistent for the optimised system. 
As this method only includes the electrostatic contribution, an obvious limitation to this technique is that any reactions involving direct interaction with solvent molecules, such as stablisation by solvation shells \textit{via} van der Waals or hydrogen bonding interactions, will be missed. In these cases the explicit solvent molecules should be added. 
However, for models where explicit interaction with the solvent is not required but solvent dieletric properties strongly influence the reaction environment, these methods can be very effective. 
This method has demonstrated successful application in the study of cellulose and glucose reactions in solvent \cite{Mayes2012,Li2018}. 
For calculations in solvent, the SCRF=PCM keyword will therefore be included in \ac{G09} calculation specifications, with water ($\epsilon$=78.4) as the default solvent.
}
...
A quantitative way to obtain information on the topology of the electron density, is by taking the first derivative of the gradient ($\nabla(\rho)$). 
